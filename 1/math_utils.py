...
def gen_adj_matrix_df(df):
    adj_matrix_df = pd.DataFrame(index=range(
        df.shape[0]), columns=range(df.shape[0]))
    for i in range(df.shape[0]):
        for j in range(i, df.shape[0]):
            if j == i:
                adj_matrix_df.at[i, j] = 0.0
                continue
            vi = pd.DataFrame(index=range(df.shape[1]), columns=range(1))
            vj = pd.DataFrame(index=range(df.shape[1]), columns=range(1))
            for k in range(df.shape[1]):
                vi.at[k, 0] = df.at[i, k]
                vj.at[k, 0] = df.at[j, k]
            adj_matrix_df.at[i, j] = distance_of_column_vector_df(vi, vj)
            adj_matrix_df.at[j, i] = adj_matrix_df.at[i, j]
